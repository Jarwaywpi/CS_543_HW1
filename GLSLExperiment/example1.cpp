...
void tEventHandler(void)
{
	...
	for (int y = 0; y < 5; y++)
	{
		for (int x = 0; x < 5; x++)
		{
				int w = (1.067*(height - 0.08*width)) / 5;
				int h = (height - 0.08*width) / 5;
				int leftOffset = (width - 5 * w) / 2;
				int topOffset = (height - 5 * h) / 2;
				glViewport(leftOffset + w*x, h*y, 0.8*w, 0.8*h);
...
void fernFractal(void)
{
	...
	pointsFern[0].x = float(rand() % 100) / 100.0;
	pointsFern[0].y = float(rand() % 100) / 100.0;
